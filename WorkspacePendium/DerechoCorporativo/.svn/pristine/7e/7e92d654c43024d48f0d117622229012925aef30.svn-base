...
/**
 * Servlet implementation class RepHistorialCorpServlet
 */
@WebServlet("/RepHistorialCorpServlet")
public class RepHistorialCorpServlet extends HttpServlet {
	...
	public void processRequest(HttpServletRequest request,
		...
		try {
			/*if(txtSalida.equals("PDF")){
				viewPDF("reporteHistorialCorporativo", request, response,lsEmpresas,lsResponsable);
			}else if(txtSalida.equals("EXCEL")){
				viewExcel("reporteHistorialCorporativo", request, response,lsEmpresas,lsResponsable);
			}*/
			if(txtSalida.equals("PDF")){
				if(lsEmpresas.equals("") && lsResponsable.equals("") && lstxSolicitadoPor.equals("") && 
				 ...
				 if((lsResponsable.indexOf(",") == -1) && (lsEmpresas.equals("") && 
					...
					viewPDF("reporteHistorialCorporativo2", request, response,lsEmpresas,lsResponsable);
				}
			}else if(txtSalida.equals("EXCEL")){
					if(lsEmpresas.equals("") && lsResponsable.equals("") && lstxSolicitadoPor.equals("") && 
							lstxDe.equals("") && lstxPara.equals("")){
						...
						viewExcel("reporteHistorialCorporativo2", request, response,lsEmpresas,lsResponsable);
					}
	...
	private void preCargaReporte(String nomReporte,HttpServletRequest  request,
			...
			/* Anterior
				if(lsEmpresas.length() > 0 && lsResponsable.length() > 0){
					parametros.put("p_condicion", " AND ID_EMPRESA IN("+lsEmpresas+") AND responsable IN("+lsResponsable+")");
				}else if (lsEmpresas.length() > 0){
					parametros.put("p_condicion", " AND ID_EMPRESA IN("+lsEmpresas+")");
					
				}else if(lsResponsable.length() > 0){
					parametros.put("p_condicion", " AND responsable IN("+lsResponsable+")");
				}else{
     ...
     */
    public RepHistorialCorpServlet() {
        super();
