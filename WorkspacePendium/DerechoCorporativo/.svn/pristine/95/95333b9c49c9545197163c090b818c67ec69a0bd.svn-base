...
public class ExportExcelPoderesServlet extends HttpServlet{
		...
		if(!escrituraFilter.equals("")){
			...
			if(gdb.beans.size() > 0){	
				for(int i=0;i<gdb.beans.size();i++){
					...
					String descTipoPoder = (tipoEscrituraActual.equals("PG") ? "Poder General" : tipoEscrituraActual.equals("PE") ? "Poder Especial" : tipoEscrituraActual.equals("CP") ? "Carta Poder" : "Escritura de Revocación");
	...
	public String removeHTMLTags(String textoHTML){
		...
		textoPlano = textoHTML.replaceAll("&ntilde;", "ñ");
		textoPlano = textoHTML.replaceAll("&Ntilde;", "Ñ");
