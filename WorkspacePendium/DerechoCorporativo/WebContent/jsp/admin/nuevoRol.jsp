...
function validaForm(){
	...
	if(nombre == ''){
		swal("Es requerido capturar el Nombre del Rol")
			...
			document.getElementById("frmNvoRol").submit();
		}
}
...
<form id="frmNvoRol" name="frmNvoRol" method="post" action="/DerechoCorporativo/faces/GuardaRolServlet?action=nuevo">
<p>&nbsp;</p>
<p>&nbsp;</p>
<% RolesDAO rolesDAO = new RolesDAO();
   List<MenuBean> listMenu = rolesDAO.getNewMenuElements(1);
   List<PestaniaBean> listPest  = rolesDAO.getPestanias(1);
 %>
<table width="80%" border="0" align="center" cellspacing="0" cellpadding="0">
  <tr style="background-image: url('/DerechoCorporativo/img/Back2.png')">
    <th width="100%" align="center"><font style="color: white;"> Nuevo Rol </font></th>
  ...
  <tr>
    ...
    <td>Nombre:</td>
    <td>
      <input type="text" name="txtNombre" id="txtNombre" maxlength = "20" />
    </td>
  </tr>
  <tr>
    <td>Descripci&oacute;n:</td>
    <td><input type="text" name="txtDesc" id="txtDesc" maxlength = "20"/></td>
  </tr>
  <tr>
    <td>Expiraci&oacute;n Password (D&iacute;as)</td>
    <td><input type="text" name="txtExp" id="txtExp" onkeypress="return isNumberKey(event)" /></td>
  </tr>
  	</table>
  	</td>
  </tr>
...
<%    for(MenuBean menu: listMenu){ %>
		<tr>
		<td><input type="checkbox" class="check_secciones" id="chk_<%=menu.getIdMenuElement() %>_<%=menu.getIdMenuElementParent() %>" name="chk_<%=menu.getIdMenuElement() %>" value="1" onclick="updateCheck(this);" ><%=menu.getNomMenu() %><br></td>
		</tr>
<%	  } %>
  	</table>
  	</td>
...
<%	for(PestaniaBean pestania: listPest){   	%>	
		<tr>
		<td width="100%"><input type="checkbox" class="check_pest" id="chkp_<%=pestania.getIdSeccion()%>_<%=pestania.getIdRol()%>" 
			name="chkp_<%=pestania.getIdSeccion()%>_<%=pestania.getIdRol()%>" value="1"	>
...
<%	} %>
  ...
  <tr>
  	...
  	<table width="80%" border="0">
		...
		<tr>
		<td width="100%">
		<%= MultiSelectList.getMultiSelectListEmp("", "0") %>
		</td>
		<td>
  ...
  <tr>
  	...
  	<table width="80%" border="0">
		...
		<tr>
		<td width="100%">
		<%= MultiSelectList.getMultiSelectListReportPre("", "0") %>
		</td>
		<td>
  ...
  <tr>
  	...
  	<table width="80%" border="0">
		...
		<tr>
		<td width="100%">
		<%= MultiSelectList.getMultiSelectListReportPer("", "0") %>
		</td>
		<td>
