...
<%
	{
		...
		//JJAQ
				...
				if(isRevocado != null && isRevocado.equals("si")){
					...
					elems = new DetailApoderados().getApoderadosEsp(idEmpresa, lstNumTipoPoder, lstEscritura);		
				}
		for (DetailApoderadosDTO elem : elems) {
			alterClass = !alterClass;
			trStyle = (alterClass)?"class=\"tableRow2\"" : "";
%>		<tr <%=trStyle%> >
			<td><%= elem.getApoderados().replace("(","<br><font color='#555555'>(").replace(")", ")</font>").replace("<br><!-- X1 -->", "<UL>").replace("<br><!-- X2 -->", "</UL>") %></td>
