...
public class ReporteAdmVigFlexTable {
	
	private final static Logger log = Logger.getLogger(ReporteAdmVigFlexTable.class);
	...
	ArrayList<ReporteAdmVigFlexColumn> flexColumns;
	...
	public ReporteAdmVigFlexTable(String idflexTbl,String lstIdEmpresa) {
	...
	public ReporteAdmVigFlexTable(int idflexTbl,String lstIdEmpresa) {

		init(idflexTbl,lstIdEmpresa);
	}

	public ReporteAdmVigFlexTable(ResultSet set) throws SQLException {

		ReflexionUtil.fillObject(set, set.getMetaData(), this, ReporteAdmVigFlexTable.class);
	}
	...
	public static ArrayList<ReporteAdmVigFlexTable> getReformas() {

		ArrayList<ReporteAdmVigFlexTable> fx = new ArrayList<ReporteAdmVigFlexTable>();
		...
		try {
			...
			while (set.next()) {
				...
				fx.add(new ReporteAdmVigFlexTable(set));
			}
	...
	public String toHTMLHorizontal(HttpServletRequest request,String lstIdEmpresa,String accionistas,String nombresFunc) {
		...
		try {
			...
			boolean reporte = false;
				flexColumns = ReporteAdmVigFlexColumn.getFlexColumns(this.idflexTbl, conn,
			...
			if (isConsulta && this.atributo13 != null
				...
				for (ReporteAdmVigFlexColumn flexColumn : flexColumns) {
				...
				if (filterRowsForReport) {
					...
					for (Entry<String, String> entry : map.entrySet()) {
						...
						if (Integer.parseInt(key) >= 10000) {
							...
							ReporteAdmVigFlexColumn fc = ReporteAdmVigFlexColumn.getFlexColumn(
									idFlexColum, conn);
	...
	public String getCatalogValue(ConnectionWrapper connect,
			ReporteAdmVigFlexColumn flexColumn, String value) throws Exception {
	...
	public String getNomCatVal(CatalogoValor catElem, ReporteAdmVigFlexColumn flexColumn,
			String modo) {
	...
	public String getCatalogValue(ConnectionWrapper connect,
			ReporteAdmVigFlexColumn flexColumn, String value, int tiIdMetaRow,
	...
	public String getMultiCatalogValue(ConnectionWrapper connect,
			ReporteAdmVigFlexColumn flexColumn, String value) {
	...
	public int toExcel(	XSSFSheet sheet, 
		...
		return toExcelHorizontal(sheet, connect, request, rowIndex,lstIdEmpresa,idFlex,nomFuncionario);
	}
	...
	public int toExcelHorizontal(XSSFSheet 			sheet, 
		...
		String nomFlex = "";
		String nombreFlex = "";
		boolean tieneInfoFlex = false;
		
		if(nomFuncionario.length() > 0){
			tieneInfoFlex = EmpresaValues.tieneInfoFlexFuncionario(Integer.parseInt(lstIdEmpresa), idFlex,nomFuncionario);
		}else{
			tieneInfoFlex = EmpresaValues.tieneInfoFlex(Integer.parseInt(lstIdEmpresa), idFlex);
		}
		if(tieneInfoFlex){
			rowIndex += 1;
		}
		...
		try {
			...
			if ((stEditable == null) || (stEditable.equals(""))) {
				...
				flexColumns = ReporteAdmVigFlexColumn.getFlexColumns(idFlex, conn,
						true, request, "0", this.specificFlexColumnsCond, true,
						reporte);
				// isConsulta = false;
			} else {
				flexColumns = ReporteAdmVigFlexColumn.getFlexColumnsExcel(idFlex,
			...
			boolean showTotalsRow = false;
			Map<String, Double> totalsMap = new HashMap<String, Double>();
			nombreFlex = EmpresaValues.getNombreFlex(idFlex);
			...
			if(tieneInfoFlex){
				...
				ExcelUtil.setCellValue(sheet, rowIndex-1, 2, nombreFlex);
				ExcelUtil.setCellStyleBold(sheet, rowIndex-1, 2);
...
/*
			...
			/*
				if (filterRowsForReport) {
					...
					String key = "";
					String value = "";
					ReporteAdmVigFlexColumn fc = null;
					String code = "";
					...
					for (Entry<String, String> entry : map.entrySet()) {
						...
						if (Integer.parseInt(key) >= 10000) {
							...
							fc = ReporteAdmVigFlexColumn.getFlexColumn(idFlexColum, conn);
