...
public class NumberFormatter {
		...
		try {
			...
			if(decimalPlaces == 0) {
				...
				if(tok.hasMoreElements()) {
					decimal = StringUtils.padOrCrunch(tok.nextToken(), '0', decimalPlaces, 
							StringUtils.PAD_DERECHO);
				} else {
					decimal = StringUtils.padOrCrunch("0", '0', decimalPlaces, 
							StringUtils.PAD_DERECHO);
				}			
	...
	public static String conLetra(double numero) {
		...
		if(numero > 9999999.99) {
			...
			String decimal = StringUtils.pad(porPartes.nextToken(), '0', 2,
					StringUtils.PAD_IZQUIERDO); 
